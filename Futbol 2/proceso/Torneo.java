...
public class Torneo {
	...
	/*
	 * 	Constructor. Llama a los métodos para jugar los partidos de un torneo y
	 * 	generar sus fechas.
	 * 
	 * 	Recibe: un objeto List en el que cada uno de los índices contiene un objeto
	 * 	Equipo.
	 */
	 ...
	 * 	asignándole la condición de local a los equipos que se encuentran bajo índices
	 * 	pares y el cero. Luego por cada equipo recursa todos los equipos, y si el índice
	 * 	del array jugado[] del primer equipo correspondiente al segundo equipo contiene
	 * 	true, resuelve la localía. Si no hay conflictos, crea un nuevo Objeto partido con
	 * 	ambos equipos como parámetros. Si las localías son iguales, es local el equipo que
	 * 	menos veces jugó de local hasta el momento.
	 */
	@SuppressWarnings("unchecked")
	 ...
	 * 	los partidos en las sucesivas fechas. Cuando llega al próximo equipo (esto
	 * 	se determina por el índice partidosAAgregar, que es decreciente desde
	 * 	cantidadDeFechas -para el primer equipo hay cantidadDeFechas partidos,
	 * 	para el segundo cantidadDeFechas-1, para el tercero cantidadDeFechas-2 y así
	 * 	sucesivamente),	se empieza desde dos fechas más adelante que el equipo anterior.
	 * 	Se repite el proceso, fijándose en cada fecha que ninguno de los dos equipos haya
	 * 	jugado ya en esa fecha.
	 * 	Si jugaron ya, se pasa a la próxima fecha. Si la próxima fecha excede el límite de
	 * 	fechas cantidadDeFechas, se vuelve a la primer fecha
	 * 	(if (inicio+inc>cantidadDeFechas-1{inc=-inicio;}).
	 * 	Cuando la posición en la que se va a empezar a poner equipos es mayor al límite de 
	 * 	fechas,	a esa posición se le resta la cantidad de fechas para que empiece desde el
	 * 	principio en el próximo ciclo,
	...
	public void generarFechas(){
		 ...
		 * 	de equipos del torneo menos 1, ya que los equipos no juegan contra sí mismos.
		 */
		 ...
		 * 	todos los partidos jugados, esté vacío.
		 */
		while (!todosLosPartidos.isEmpty()){
			/*
			 * 	int inc = incremento de las fechas. Este número se incrementa en cada
			 * 	ciclo del while (partidosAgregados<partidosAAgregar). Se le suma a inicio
			 *  para determinar la fecha en la que se intentará agregar el próximo partido.
			 */
			int inc=0;
			/*
			 * 	int partidosAgregados = partidos ya agregados. Incrementa en 1 en los
			 * 	ciclos del while (partidosAgregados<partidosAAgregar) en los que se agregó
			...
			while (partidosAgregados<partidosAAgregar){
				/*
				 * 	Si la posición en la que se va a intentar agregar el próximo partido
				 * 	es mayor que el índice de la última fecha, el incremento se setea en
				...
				/*
				 * 	Se toma el partido ubicado en el índice 0 de la lista de todos los
				 ...
				 *  correspondientes al partido en el índice 0. Si alguno de los equipos
				 *  ya jugó en esta fecha, se setea el valor de saltear a true.
				 */
				...
				/*
				 * 	Si el valor de saltear es false, agrega el partido en el índice 0 de la
				 * 	lista todosLosPartidos a la fecha actual e incrementa el número de
				 * 	partidos agregados. El método remove(int n) de la clase List saca el
				 * 	objeto del índice n de la lista, y corre todos los demás índices hacia
				 * 	la izquierda (el objeto del índice 1 pasa a ser el del índice 0, el del
				 * 	2 pasa a estar en el 1, y así sucesivamente). Por lo tanto, además de
				...
				/*
				 * 	Incrementa en 1 la variable inc para que en el próximo ciclo del while
				 * 	se intente agregar el partido a la próxima fecha.
				 */
				inc++;	
			}
			/*
			 * 	Decrece la cantidad de partidos a agregar, para que en el próximo ciclo
			 ...
			 * 	tercero dos menos, y así sucesivamente. 
			 */
			partidosAAgregar--;
			/*
			 * 	Se incrementa en 2 el inicio para que en el próximo ciclo se empiece a
			 * 	agregar partidos desde dos fechas más adelante. El if asegura que no se
			 * 	intentará meter equipos en una fecha que no existe (mayor que la cantidad
			 * 	total de fechas -1).
			 */
			inicio=inicio+2;
	 ...
	 * 	Por cada equipo en la lista chequea si el próximo equipo en la lista tiene
	 * 	un puntaje mayor, y si es así, intercambia sus posiciones en la lista. Si los
	 * 	puntajes son iguales, chequea si el próximo equipo tiene una diferencia de goles
	 * 	mayor, y si es así, intercambia sus posiciones en la lista. Repite este proceso
	 ...
	 *  más puntos y mejor diferencia de gol tiene y el último el que menos.
	 * 	Devuelve: un objeto List ordenado por puntaje y diferencia de goles en el que
	 *  cada índice contiene un objeto equipo.
	 */
	@SuppressWarnings("unchecked")
	...
	/*
	 * 	Método auxiliar que recibe una lista de equipos y devuelve una lista con todos los
	 * 	jugadores de esos equipos.
	 * 	Recibe: un objeto List en el que cada índice contiene un objeto Equipo.
	 ...
	 * 	Devuelve: un objeto List en el que cada índice contiene un jugador ordenado
	 * 	por cantidad de goles en la que el primer jugador es el que más goles tiene
